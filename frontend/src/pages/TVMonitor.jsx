...
export default function TVMonitor() {
  ...
  const user = null; // No user for TV
  const isITStaff = false;
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`${API_BASE}/tv/tickets`);
  ...
  useEffect(() => {
    fetchRequests();
    const interval = setInterval(fetchRequests, 60000); // Auto refresh every 60 seconds
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      case 'Completed': return 'text-emerald-700 bg-emerald-100/50 ring-emerald-600/20';
      case 'In Progress': return 'text-indigo-700 bg-indigo-100/50 ring-indigo-600/20';
      case 'Rejected': return 'text-rose-700 bg-rose-100/50 ring-rose-600/20';
      case 'Cancelled': return 'text-slate-600 bg-slate-100/50 ring-slate-500/20';
      case 'Pending': return 'text-amber-700 bg-amber-100/50 ring-amber-600/20 animate-pulse';
      default: return 'text-amber-700 bg-amber-100/50 ring-amber-600/20';
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 text-slate-900 font-sans flex overflow-hidden">
      ...
      {/* Sidebar is removed for TV monitor */}
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 mb-5 grid grid-cols-2 md:grid-cols-5 gap-4">
                <div>
                  ...
                  <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-base font-medium text-slate-700 focus:outline-none focus:border-indigo-500">
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-in fade-in duration-300">
                {paginatedRequests.map((req) => (
                  <div
                    key={req.id}
                    className={`relative border rounded-3xl p-6 shadow-sm flex flex-col justify-between group overflow-hidden ${
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                        {req.req_id}
                        {(req.status === 'Pending' || !req.empCode_assigned) && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-black bg-orange-500 text-white shadow-sm animate-pulse">
                            NEW
                          </span>
                        )}
                      </p>
                    ...
                    <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2" title={`Requester: ${req.requester_name}`}>
                          ...
                          <span className="text-[11px] font-bold text-amber-500 max-w-[60px] truncate">{req.requester_name.split(' ')[0]}</span>
                        </div>
                      ...
                      <div className="text-right">
                        <p className="text-[11px] font-black text-slate-500">Due: {formatDateShort(req.target_date)}</p>
                        {req.accepted_at && req.completed_at ? (
                          <p className="text-[12px] font-bold text-emerald-500 mt-0.5">⏱ {formatElapsedTime(req.accepted_at, req.completed_at)}</p>
                        ) : req.accepted_at ? (
                          <p className="text-[12px] font-bold text-indigo-500 mt-0.5">In progress...</p>
                        ) : null}
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-x-auto animate-in fade-in duration-300">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-slate-50/80 text-slate-800 text-[15px] uppercase font-black tracking-widest border-b border-slate-100">
                    <tr>
                      <th className="px-6 py-4 cursor-pointer hover:text-slate-600 transition-colors" onClick={() => handleSort('priority')}>Priority<SortIcon field="priority" /></th>
                      <th className="px-6 py-4 cursor-pointer hover:text-slate-600 transition-colors" onClick={() => handleSort('project_name')}>Project Name<SortIcon field="project_name" /></th>
                      <th className="px-6 py-4">Category</th>
                      <th className="px-6 py-4 cursor-pointer hover:text-slate-600 transition-colors" onClick={() => handleSort('status')}>Status<SortIcon field="status" /></th>
                      <th className="px-6 py-4">Location</th>
                      <th className="px-6 py-4">Requester</th>
                      <th className="px-6 py-4">In Charge</th>
                      <th className="px-6 py-4 text-right cursor-pointer hover:text-slate-600 transition-colors" onClick={() => handleSort('target_date')}>Target Date<SortIcon field="target_date" /></th>
                    ...
                    {paginatedRequests.map((req) => (
                      ...
                      >
                        ...
                        <td className={`px-6 py-3 ${req.status === 'Pending' || !req.empCode_assigned ? 'border-l-[6px] border-l-orange-500' : 'border-l-[6px] border-l-transparent'}`}>
                          <span className={`inline-flex px-3 py-1.5 rounded-lg text-[14px] font-black uppercase tracking-widest border ${getPriorityStyle(req.priority)}`}>
                            {req.priority}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-3 pr-2">
                          <p className="text-lg font-bold text-slate-800 group-hover:text-indigo-600 transition-colors truncate max-w-[300px]">{req.project_name}</p>
                        ...
                        <td className="px-6 py-3">
                          <span className={`inline-flex items-center gap-2 text-sm font-bold px-3 py-1.5 rounded-xl w-fit border shadow-sm
                            ${getDeviceTypeColor(requestTypeMap[req.request_type] || 'office').bg}
                            ${getDeviceTypeColor(requestTypeMap[req.request_type] || 'office').text}
                            ${getDeviceTypeColor(requestTypeMap[req.request_type] || 'office').border}`}>
                            {getRequestTypeIcon(req.request_type)}
                        ...
                        {/* Status */}
                        <td className="px-6 py-3">
                          <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm font-black uppercase tracking-wider ring-1 ring-inset ${getStatusStyle(req.status)}`}>
                        ...
                        <td className="px-6 py-3">
                          <span className="flex items-center gap-1.5 text-sm font-bold text-slate-700">
                            <MapPin className="w-4 h-4 shrink-0 text-slate-700" /> {req.location}
                          </span>
                        </td>

                        {/* Requester */}
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-4" title={`Requester: ${req.requester_name}`}>
                            <img
                              src={req.emp_pic_url || `http://dcidmc.dci.daikin.co.jp/PICTURE/${req.empCode_created}.jpg`}
                              alt={req.requester_name}
                              className="w-16 h-18 rounded-lg object-cover ring-2 ring-slate-200 shadow-lg bg-slate-50"
                              style={{ imageRendering: 'auto' }}
                              loading="lazy"
                              onError={(e) => {
                                e.target.onerror = null;
                                e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(req.requester_name)}&background=e0e7ff&color=4f46e5&size=64`;
                              }}
                            />
                            <span className="text-[15px] font-black text-slate-800 max-w-[150px] truncate">{req.requester_name?.split(' ')[0]}</span>
                          </div>
                        </td>

                        {/* In Charge */}
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-4" title={req.empCode_assigned ? `In Charge: ${req.assigned_name}` : 'Unassigned'}>
                            {req.empCode_assigned ? (
                              <>
                                <img
                                  src={`http://dcidmc.dci.daikin.co.jp/PICTURE/${req.empCode_assigned}.jpg`}
                                  alt={req.assigned_name}
                                  className="w-16 h-18 rounded-lg object-cover ring-4 ring-indigo-200 shadow-lg bg-indigo-50"
                                  style={{ imageRendering: 'auto' }}
                                  loading="lazy"
                                  onError={(e) => {
                                    e.target.onerror = null;
                                    e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(req.assigned_name || req.empCode_assigned)}&background=dcfce7&color=166534&size=64`;
                                  }}
                                />
                                <span className="text-[15px] font-black text-indigo-700 max-w-[150px] truncate">{req.assigned_name?.split(' ')[0] || req.empCode_assigned}</span>
                              </>
                            ) : (
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-[15px] font-bold text-slate-500">ยังไม่มีคนรับ</span>
                              </div>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-3 text-right">
                          <p className="text-base font-bold text-slate-600">{formatDateShort(req.target_date)}</p>
                          {req.accepted_at && req.completed_at ? (
                            <p className="text-sm font-bold text-emerald-500 mt-1">⏱ {formatElapsedTime(req.accepted_at, req.completed_at)}</p>
                          ) : req.accepted_at ? (
                            <p className="text-sm font-bold text-indigo-500 mt-1">In progress...</p>
                          ) : null}
                        </td>

                        <td className="px-6 py-3 text-right">
                          <button onClick={() => setSelectedTicket(req)} className="text-slate-400 hover:text-indigo-600 p-2 rounded-full hover:bg-indigo-100/50 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 group-hover:translate-x-1 transition-transform"><path d="m9 18 6-6-6-6"/></svg>
                          </button>
                        </td>
                      </tr>
                    ))}
